<div class="container-fluid px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-4">
            <table id="tahunAjaranTable" class="display w-full">
                <thead>
                    <tr>
                        <th>No</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?= $no++ ?></td>
                        <td class="font-semibold"><?= $ta->nama_tahun ?></td>
                        ...
                        <td>
                            <div class="flex gap-2">
                                <?php if(!$ta->is_active): ?>
                                <a href="<?= base_url('admin/tahunajaran/set_active/'.$ta->id) ?>" 
                                   class="text-green-600 hover:text-green-800" 
                                   onclick="return confirm('Aktifkan tahun ajaran ini?')">
                                    <i class="fas fa-check-circle"></i>
                                </a>
                                <?php endif; ?>
                                <button onclick="editData(<?= $ta->id ?>)" class="text-blue-600 hover:text-blue-800">
...
<div id="formModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 id="modalTitle" class="text-lg font-semibold">Tambah Tahun Ajaran</h3>
        ...
        <form id="tahunAjaranForm">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Tahun Mulai *</label>
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Tahun Selesai *</label>
...
function editData(id) {
    $.ajax({
        url: '<?= base_url("admin/tahunajaran/get/") ?>' + id,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.status == 'success') {
                ...
                $('#tahun_selesai').val(response.data.tahun_selesai);
...
function deleteData(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '<?= base_url("admin/tahunajaran/delete/") ?>' + id,
                type: 'POST',
                dataType: 'json',
                success: function(response) {
                    if (response.status == 'success') {
...
$('#tahunAjaranForm').on('submit', function(e) {
    ...
    var id = $('#edit_id').val();
    var url = id ? '<?= base_url("admin/tahunajaran/edit/") ?>' + id : '<?= base_url("admin/tahunajaran/add") ?>';
