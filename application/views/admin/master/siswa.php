    ...
    <div class="p-4 mt-14">
        ...
        <div class="mb-6 flex justify-between items-center">
            <div>
                ...
                <p class="text-gray-600 mt-1">Kelola data siswa dan RFID</p>
            </div>
            <div class="flex space-x-2">
                ...
                <a href="<?= base_url('admin/siswa/export') ?>" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="overflow-x-auto">
                <table id="siswaTable" class="datatable w-full text-sm text-left text-gray-500">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3">No HP Ortu</th>
                    ...
                    <tbody>
                        <?php $no = 1; foreach ($siswa as $s): ?>
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4"><?= $s['no_hp_ortu'] ?></td>
                                <td class="px-6 py-4">
                                    <?php if ($s['is_active']): ?>
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">
                                            Aktif
                                ...
                                <td class="px-6 py-4">
                                    <button onclick="editSiswa(<?= $s['id'] ?>)" class="text-blue-600 hover:text-blue-900 mr-2" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button onclick="confirmDelete(<?= $s['id'] ?>, '<?= $s['nama'] ?>')" class="text-red-600 hover:text-red-900" title="Delete">
...
<div id="siswaModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold" id="modalTitle">Tambah Siswa</h3>
        ...
        <form id="siswaForm" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- NIS -->
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">NIS <span class="text-red-500">*</span></label>
                    <input type="text" name="nis" id="nis" required class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                
                <!-- NISN -->
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">NISN</label>
                    <input type="text" name="nisn" id="nisn" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                ...
                <!-- Jenis Kelamin -->
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">Jenis Kelamin <span class="text-red-500">*</span></label>
                    <select name="jenis_kelamin" id="jenis_kelamin" required class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <!-- Kelas -->
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">Kelas <span class="text-red-500">*</span></label>
                    <select name="kelas_id" id="kelas_id" required class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <!-- No HP Siswa -->
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">No HP Siswa</label>
                    <input type="text" name="no_hp_siswa" id="no_hp_siswa" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                
                <!-- Nama Orang Tua -->
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">Nama Orang Tua</label>
                    <input type="text" name="nama_ortu" id="nama_ortu" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                
                <!-- No HP Orang Tua -->
                <div>
                    <label class="block text-gray-700 text-sm font-medium mb-2">No HP Orang Tua (WA)</label>
                    <input type="text" name="no_hp_ortu" id="no_hp_ortu" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="08xxxxxxxxxx">
                </div>
...
<div id="importModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        ...
        <form action="<?= base_url('admin/siswa/import') ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-medium mb-2">File Excel (.xlsx)</label>
            ...
            <div class="bg-blue-50 p-3 rounded-lg mb-4">
                ...
                <p class="text-xs text-blue-700">Kolom: NIS, NISN, RFID UID, Nama, JK (L/P), Tempat Lahir, Tgl Lahir (YYYY-MM-DD), Alamat, No HP Siswa, Nama Ortu, No HP Ortu, Email, Kelas ID</p>
            </div>
    ...
    fetch(base_url + 'admin/siswa/get/' + id)
        ...
        .then(data => {
            ...
            if (data.status) {
                ...
                document.getElementById('no_hp_siswa').value = data.data.no_hp_siswa;
                document.getElementById('nama_ortu').value = data.data.nama_ortu;
                document.getElementById('no_hp_ortu').value = data.data.no_hp_ortu;
...
function confirmDelete(id, nama) {
    Swal.fire({
        ...
        text: `Hapus siswa "${nama}"? Data tidak dapat dikembalikan!`,
        icon: 'warning',
...
document.getElementById('siswaForm').addEventListener('submit', function(e) {
    ...
    const url = siswaId ? base_url + 'admin/siswa/edit/' + siswaId : base_url + 'admin/siswa/add';
